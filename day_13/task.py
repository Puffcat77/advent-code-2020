...
# from the sea port at timestamps 0, 5, 10, 15, and so on.
...
def partOne(data):
    departureTime, busIDs = int(data[0]), [int(d) for d in data[1].replace(',x', '').split(',')]
    minTime = -1
    busID = -1
    for id in busIDs:
        nearTime = (departureTime // id + 1) * id
        delta = nearTime - departureTime
        if minTime == -1 or minTime > delta:
            minTime = delta
            busID = id
...
# For example, in the list above, because bus ID 19 must depart seven minutes after the
# timestamp at which bus ID 7 departs, bus ID 7 will always also be departing with bus ID 19
...
def countWithCRT(stamps):
    ...
    for i in range(len(equations)):
        ...
        m_i = int(m / divisor)
        reverse = 1
        for r in range(1, divisor + 1):
            if (m_i * r) % divisor == remainder:
                reverse = r
                break
        x += (reverse * m_i) % m
    x = x % m
...
def reformatDeltas(stamps):
    res = {}
    for i in range(len(stamps)):
        if stamps[i] != 'x':
            res[int(stamps[i])] = i
    maxElem = max(res.keys())
    maxElemIndex = res[maxElem]
    res[maxElem] = 0
    for i in range(len(stamps)):
        if stamps[i] == 'x':
            continue
        if i < maxElemIndex:
            res[int(stamps[i])] = i - maxElemIndex + 1
        if i > maxElemIndex:
            res[int(stamps[i])] = i - maxElemIndex - 1
    return res
...
def partTwo(data):
    stamps = [d for d in data[1].split(',')]
    return countWithCRT(stamps)
