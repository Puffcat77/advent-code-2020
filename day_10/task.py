...
# forecasts of a massive tropical storm. Before you can figure out whether
# it will impact your vacation plans, however, your device suddenly turns off!
#
# Its battery is dead.
#
...
# jolts higher than the highest-rated adapter in your bag. (If your adapter
# list were 3, 9, and 6, your device's built-in adapter would be rated for 12 jolts.)
#
...
# outlet to your device's built-in adapter and count the joltage differences between the
...
def countJoltsWithDifference(data):
    prev = data[0]
    jolts = [1, 2, 3]
    res = [0, 0, 0]
...
def partOne(data):
    ...
    jolts = countJoltsWithDifference(data)
    return jolts[0] * jolts[2]


# --- Part Two ---
# To completely determine whether you have enough adapters, you'll need to figure out
# how many different ways they can be arranged. Every arrangement needs to connect
...
# Given the adapters from the first example, the total number of arrangements that connect the
...
# there must be more than a trillion valid ways to arrange them! Surely, there must be an efficient
# way to count the arrangements.
#
# What is the total number of distinct ways you can arrange the adapters to connect the charging outlet to your device?
def getContinuousParts(data):
    res = [[]]
    j = 0
    for i in range(1, len(data)):
        d = data[i]
...
def partTwo(data):
    parts = getContinuousParts(data)
    total = 1
    for p in parts:
        if len(p) > 1:
            permCount = countPermutations(p)
            total *= permCount
    return total
