...
# those that have all required fields. Treat cid as optional. In your batch file, how many passports are valid?
...
def checkDate(param, min, max):
    return min <= int(param) <= max


def checkHgt(param):
    if param.isdigit():
        return False
    value, system = int(param[0:-2]), param[-2:]
    return (system == 'in' and 59 <= value <= 76 or
            system == 'cm' and 150 <= value <= 193)


def checkHcl(param):
    return (param[0] == '#' and len(param) == 7 and
            param[1:].isalnum())


def checkEcl(param):
    validValues = ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth']
    return param in validValues


def checkPid(param):
    return param.isdigit() and len(param) == 9
...
def partTwo(passports):
    ...
    for p in passports:
        if len(p.keys()) == 8 or (len(p.keys()) == 7 and 'cid' not in p.keys()):
            if (checkDate(p['byr'], 1920, 2002) and  # four digits; at least 1920 and at most 2002.
                    checkDate(p['iyr'], 2010, 2020) and  # four digits; at least 2010 and at most 2020.
                    checkDate(p['eyr'], 2020, 2030) and  # four digits; at least 2020 and at most 2030.
                    checkHgt(p['hgt']) and  # a number followed by either cm or in:
                    # If cm, the number must be at least 150 and at most 193.
                    # If in, the number must be at least 59 and at most 76.
                    checkHcl(p['hcl']) and  # a # followed by exactly six characters 0-9 or a-f.
                    checkEcl(p['ecl']) and  # one of: amb blu brn gry grn hzl oth
                    checkPid(p['pid'])):  # a nine-digit number, including leading zeroes
                validPassports += 1
    return validPassports


def parseLinesToPassports(l):
    data = l.split()
    passport = {}
    for d in data:
        param, value = d.split(':')
