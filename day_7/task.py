...
# being enforced about bags and their contents;
...
bagForSearch = 'shiny gold'


def countOnlyGoldenBags(instructions, bag):
    res = 0
    for k in instructions[bag].keys():
        amount = instructions[bag][k]
        if k != bagForSearch:
            res += amount * countOnlyGoldenBags(instructions, k)
        else:
            res += instructions[bag][k]
    instructions[bag] = {bagForSearch: res}
    return res


def countGoldBags(parsedInstructions):
    count = 0
    for key in parsedInstructions.keys():
        count += 1 if countOnlyGoldenBags(parsedInstructions, key) > 0 else 0
    return count
...
def partOne(data):
    parsedInstructions = {}
    for d in data:
        bag, content = d.replace('.', '').split(' bags contain ')
        parsedInstructions[bag] = {}
        for c in content.split(', '):
            if c != 'no other bags':
                bags = ' '.join(c.split()[1:]).replace(' bags', '').replace(' bag', '')
                amount = int(c.split()[0])
                parsedInstructions[bag][bags] = amount
            else:
                parsedInstructions[bag][bagForSearch] = 0
    return countGoldBags(parsedInstructions)
...
# gold = 2 dark red + 2 * (2 dark orange + 2 * (
# 2 dark yellow + 2 * (2 dark green + 2 * (2 dark blue + 2 * (2 dark violet)))))
emptyBagIndicator = 'bags'


def countInnerBags(instructions, bag):
    res = 0
    for k in instructions[bag].keys():
        amount = instructions[bag][k]
        if k != emptyBagIndicator:
            res += amount + amount * countInnerBags(instructions, k)
        else:
            res += instructions[bag][k]
    instructions[bag] = {emptyBagIndicator: res}
    return res


def countAllInnerBags(parsedInstructions):
    countInnerBags(parsedInstructions, bagForSearch)
    return parsedInstructions[bagForSearch][emptyBagIndicator]
    ...
    for d in data:
        ...
        for c in content.split(', '):
            if c != 'no other bags':
                ...
                parsedInstructions[bag][emptyBagIndicator] = 0
    return countAllInnerBags(parsedInstructions)
