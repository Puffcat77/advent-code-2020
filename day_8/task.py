...
def countRoute(data):
    acc = 0
    isExecuted = [False] * len(data)
    i = 0
    while i < len(data) and not isExecuted[i]:
        isExecuted[i] = True
        d = data[i]
        command, value = d.split()
        sign, value = value[0], int(value[1:])
        if command == 'acc':
            acc += value if sign == '+' else (-1 * value)
        if command == 'jmp':
            i += value if sign == '+' else (-1 * value)
        else:
            i += 1
        if command == 'nop':
            continue
...
# the instruction below the last instruction in the file.
...
# имеется программма; массив состояний о том, пройден ли уже шаг; массив с маршрутом шагов программы;
# массив состояний о том, менялся ли шаг программы.
# по массиву маршрута надо идти от конца к началу: если шаг программы менялся, но цикл не пропал,
# то надо вернуть прежнее значение шага, и перейти к следующей итерации изменения программы
def getRoute(script):
    isExecuted = [False] * len(script)
    i = 0
    route = []
    while i < len(script):
        if isExecuted[i]:
...
def getAccWithoutLoop(data: []):
    lastChange = -1
    hasChanged = False
    route = getRoute(data)
    path, hasLoop = route['route'], route['hasLoop']
    while hasLoop:
        if hasChanged:
            lastChange -= 1
        else:
            command = path.pop(lastChange)
            index = command['index']
        data[index] = changeCommand(command)
        hasChanged = not hasChanged
        route = getRoute(data)
        path, hasLoop = route['route'], route['hasLoop']
    return countRoute(data)


def changeCommand(command):
    return ('jmp' if command['command'] == 'nop' else 'nop') + ' ' \
                              + command['sign'] + str(command['value'])


def partTwo(data):
    acc = getAccWithoutLoop(data)
    return acc


def task(data):
    # print(countRoute(data))
    print(partTwo(data))
