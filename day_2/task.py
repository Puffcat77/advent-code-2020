...
def isCorrect(password, letter, minCount, maxCount):
    count = 0
    for l in password:
        if count > maxCount:
            return False
        if l == letter and count <= maxCount:
            count += 1
    return maxCount >= count >= minCount


def partOne(data):
    """
    counts amount of correct passwords according to letter count in them
    :param data: array of passwords with check rules
    :return: correct passwords amount
    """
    result = 0
    for d in data:
        check, password = d.split(':')
        password = password.replace(' ', '')
        count, letter = check.split()
        minCount, maxCount = map(int, count.split('-'))
        if isCorrect(password, letter, minCount, maxCount):
            result += 1
    return result
# --- Part Two ---
...
def isCorrect2(password, letter, fst, snd):
    return (password[fst] == letter and password[snd] != letter or
            password[fst] != letter and password[snd] == letter)
...
def partTwo(data):
    ...
    for d in data:
        ...
        password = password.replace(' ', '')
        count, letter = check.split()
        firstPos, secondPos = [int(x) - 1 for x in count.split('-')]
        if isCorrect2(password, letter, firstPos, secondPos):
            result += 1
    return result
