...
# (after the preamble) which is not the sum of two of the 25 numbers before it. What is the first number that
# does not have this property?
import math


def isValid(queue, elem):
    for i in queue:
        for j in queue:
            if i != j:
                if i + j == elem and i != j:
...
def partOne(data, preambleSize):
    queue = []
    for i in range(len(data)):
        elem = int(data[i])
        if len(queue) < preambleSize:
            queue.append(elem)
        if len(queue) >= preambleSize and elem not in queue:
            if not isValid(queue, elem):
                return elem
            queue.pop(0)
            queue.append(elem)
    return -1
...
# идем с 0 элемента, вторым индексом проходим оставшуюся часть массива. Если имеющаяся сумма меньше числа,
# то идем дальше. Если сумма больше, то из списка учтенных индексов убираем последний и помечаем его как проверенный
def partTwo(data, preambleSize):
    invalid = partOne(data, preambleSize)
    for i in range(len(data)):
        d = data[i]
        if d != invalid and (invalid >= d):
            sum = d
            for j in range(i + 1, len(data)):
                another = data[j]
                if another != invalid and (sum <= invalid - another):
                    sum += another
                    if sum == invalid:
                        res = data[i: j + 1]
...
def task(data):
    ...
    preambleSize = 25
    print(partOne(data, preambleSize))
    print(partTwo(data, preambleSize))
